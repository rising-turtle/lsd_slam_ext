...
void ROSOutput3DWrapper::publishKeyframe(FrameSE3* f)
{
	...
	int h = f->height(publishLvl);
        ...
        SE3 se3_p = f->getScaledCamToWorld(); 
        Sim3 sim3_p = sim3FromSE3(se3_p, 1);
        memcpy(fMsg.camToWorld.data(), sim3_p.cast<float>().data(), sizeof(float)*7);
	// memcpy(fMsg.camToWorld.data(),f->getScaledCamToWorld().cast<float>().data(),sizeof(float)*6);
        // fMsg.camToWorld.data()[6] = 1.;
        {
                // float* p = f->getScaledCamToWorld().cast<float>().data();
          SE3 camToWorld =  f->getScaledCamToWorld();// se3FromSim3(f->getScaledCamToWorld());
          geometry_msgs::PoseStamped pMsg;

          if(f->b_has_gt_pose)
          {
            Sophus::SE3f gp = f->gt_pose; 
            ROS_WARN("ROSOutput3DWrapper.cpp: frame %d at %lf has gt %f %f %f %f %f %f %f", f->id(), f->timestamp(), gp.translation()[0], gp.translation()[1], 
                gp.translation()[2], gp.unit_quaternion().x(), gp.unit_quaternion().y(), gp.unit_quaternion().z(), gp.unit_quaternion().w());
            // camToWorld = toSophus(gp);
          }
          ...
          static int key_frame_id = 0;
          static int last_kf_id = -1;
          pMsg.header.stamp = ros::Time(f->timestamp());
          pMsg.header.frame_id = "world";
          ROS_WARN("ROSOutput3DWrapper.cpp: key frame %d index id %d at %lf publish pos_3d : %f %f %f %f %f %f %f", ++key_frame_id, f->id(), f->timestamp(), pMsg.pose.position.x, pMsg.pose.position.y, pMsg.pose.position.z,
              pMsg.pose.orientation.x, pMsg.pose.orientation.y, pMsg.pose.orientation.z, pMsg.pose.orientation.w);
          
      
          // record keyframe id and corresponding x, y, z, qx, qy, qz, qw 
          static ofstream ouf("./lsd_slam/key_frame_pose.log"); 
          static ofstream ouf2("./lsd_slam/key_frame_trajectory.log");
          if(last_kf_id < 0 || f->id() > last_kf_id)
          {
            ouf<<f->id() <<endl;
            last_kf_id = f->id();
            ouf2<<f->timestamp()<<" "<<pMsg.pose.position.x<<" " <<pMsg.pose.position.y<<" "<< pMsg.pose.position.z<<" "<<
              pMsg.pose.orientation.x<<" "<< pMsg.pose.orientation.y<<" "<< pMsg.pose.orientation.z<<" "<< pMsg.pose.orientation.w<<endl;
          }
          
            // <<" "<<pMsg.pose.position.x<<" "<<pMsg.pose.position.y<<" "<<pMsg.pose.position.z<<" "<< pMsg.pose.orientation.x<<" " <<pMsg.pose.orientation.y<<" "<<pMsg.pose.orientation.z<<" "<<pMsg.pose.orientation.w<<endl;
        }
...
void ROSOutput3DWrapper::publishKeyframe(Frame* f)
{
...

  {
	...
	pMsg.header.stamp = ros::Time(f->timestamp());
	pMsg.header.frame_id = "world";
       ROS_WARN("ROSOutput3DWrapper.cpp: key frame %d publish pos_3d : %f %f %f %f %f %f %f", ++key_frame_id, pMsg.pose.position.x, pMsg.pose.position.y, pMsg.pose.position.z,
          pMsg.pose.orientation.x, pMsg.pose.orientation.y, pMsg.pose.orientation.z, pMsg.pose.orientation.w);
}
...
void ROSOutput3DWrapper::publishTrackedFrame(FrameSE3* kf)
{
	lsd_slam_viewer::keyframeMsg fMsg;
	...
	fMsg.isKeyframe = false;
        ...
        // seems fMsg.camToWorld.data() = "x, y, z, s, r1, r2, r3";
	/*memcpy(fMsg.camToWorld.data(),kf->getScaledCamToWorld().cast<float>().data(),sizeof(float)*3);
        fMsg.camToWorld.data()[3] = 1.;
        fMsg.camToWorld.data()[4] = kf->getScaledCamToWorld().cast<float>().data()[3];
        fMsg.camToWorld.data()[5] = kf->getScaledCamToWorld().cast<float>().data()[4];
        fMsg.camToWorld.data()[6] = kf->getScaledCamToWorld().cast<float>().data()[5];
        */
        SE3 se3_p = kf->getScaledCamToWorld(); 
        Sim3 sim3_p = sim3FromSE3(se3_p, 1); 
        memcpy(fMsg.camToWorld.data(), sim3_p.cast<float>().data(), sizeof(float)*7);
	...
	SE3 camToWorld = kf->getScaledCamToWorld(); //se3FromSim3(kf->getScaledCamToWorld());
	...
	pMsg.header.frame_id = "world";
       // ROS_INFO("ROSOutput3DWrapper.cpp: frame %d publish pos_3d : %f %f %f %f %f %f %f", fMsg.id, pMsg.pose.position.x, pMsg.pose.position.y, pMsg.pose.position.z, pMsg.pose.orientation.x, pMsg.pose.orientation.y, pMsg.pose.orientation.z, pMsg.pose.orientation.w);      
...
void ROSOutput3DWrapper::publishKeyframeGraph(KeyFrameGraphSE3* graph)
{
	...
	for(unsigned int i=0;i<graph->keyframesAll.size();i++)
	{
		...
		// memcpy(framePoseData[i].camToWorld, graph->keyframesAll[i]->getScaledCamToWorld().cast<float>().data(),sizeof(float)*6);
                ...
                /*// seems fMsg.camToWorld.data() = "x, y, z, s, r1, r2, r3";
                memcpy(framePoseData[i].camToWorld,graph->keyframesAll[i]->getScaledCamToWorld().cast<float>().data(),sizeof(float)*3);
                framePoseData[i].camToWorld[3] = 1.;
                framePoseData[i].camToWorld[4] = graph->keyframesAll[i]->getScaledCamToWorld().cast<float>().data()[3];
                framePoseData[i].camToWorld[5] = graph->keyframesAll[i]->getScaledCamToWorld().cast<float>().data()[4];
                framePoseData[i].camToWorld[6] = graph->keyframesAll[i]->getScaledCamToWorld().cast<float>().data()[5];
                 */

                SE3 se3_p = graph->keyframesAll[i]->getScaledCamToWorld(); 
                Sim3 sim3_p = sim3FromSE3(se3_p, 1); 
                memcpy(framePoseData[i].camToWorld, sim3_p.cast<float>().data(), sizeof(float)*7);
	}
