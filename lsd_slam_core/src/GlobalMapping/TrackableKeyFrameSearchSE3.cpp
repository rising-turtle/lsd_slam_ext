...
TrackableKeyFrameSearchSE3::TrackableKeyFrameSearchSE3(KeyFrameGraphSE3* graph, int w, int h, Eigen::Matrix3f K)
: graph(graph)
{
...
TrackableKeyFrameSearchSE3::~TrackableKeyFrameSearchSE3()
{
	delete tracker;
}
...
std::vector<TrackableKFStructSE3, Eigen::aligned_allocator<TrackableKFStructSE3> > TrackableKeyFrameSearchSE3::findEuclideanOverlapFrames(FrameSE3* frame, float distanceTH, float angleTH, bool checkBothScales)
{
	...
	std::vector<TrackableKFStructSE3, Eigen::aligned_allocator<TrackableKFStructSE3> > potentialReferenceFrames;
	...
	for(unsigned int i=0;i<graph->keyframesAll.size();i++)
	{
		...
		potentialReferenceFrames.push_back(TrackableKFStructSE3());
...
FrameSE3* TrackableKeyFrameSearchSE3::findRePositionCandidate(FrameSE3* frame, float maxScore)
{
    std::vector<TrackableKFStructSE3, Eigen::aligned_allocator<TrackableKFStructSE3> > potentialReferenceFrames =
...
std::unordered_set<FrameSE3*, std::hash<FrameSE3*>, std::equal_to<FrameSE3*>, Eigen::aligned_allocator< FrameSE3* > > TrackableKeyFrameSearchSE3::findCandidates(FrameSE3* keyframe, FrameSE3* &fabMapResult_out, bool includeFABMAP, bool closenessTH)
{
	std::unordered_set<FrameSE3*, std::hash<FrameSE3*>, std::equal_to<FrameSE3*>, Eigen::aligned_allocator< FrameSE3* > > results;
...
FrameSE3* TrackableKeyFrameSearchSE3::findAppearanceBasedCandidate(FrameSE3* keyframe)
{
#ifdef HAVE_FABMAP
