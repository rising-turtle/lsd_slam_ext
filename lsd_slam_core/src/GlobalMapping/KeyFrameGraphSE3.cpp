...
KFConstraintStructSE3::~KFConstraintStructSE3()
{
	if(edge != 0)
		delete edge;
}

KeyFrameGraphSE3::KeyFrameGraphSE3()
: nextEdgeId(0)
{
...
{
	// deletes edges
	for (KFConstraintStructSE3* edge : newEdgeBuffer)
	...
	for(FramePoseStructSE3* p : allFramePoses)
		delete p;
}


void KeyFrameGraphSE3::addFrame(FrameSE3* frame)
{

	frame->pose->isRegisteredToGraph = true;
	FramePoseStructSE3* pose = frame->pose;
...
void KeyFrameGraphSE3::saveGraph(std::string file)
{
      graph.save(file.c_str());
}

void KeyFrameGraphSE3::dumpMap(std::string folder)
{
  /*
	...
	{
		KFConstraintStructSE3* e = edgesAll[i];
...
void KeyFrameGraphSE3::addKeyFrame(FrameSE3* frame)
{
...
void KeyFrameGraphSE3::insertConstraint(KFConstraintStructSE3* constraint)
{
...
bool KeyFrameGraphSE3::addElementsFromBuffer()
{
	bool added = false;
...
int KeyFrameGraphSE3::optimize(int num_iterations)
{
...
void KeyFrameGraphSE3::calculateGraphDistancesToFrame(FrameSE3* startFrame, std::unordered_map< FrameSE3*, int >* distanceMap)
{
	distanceMap->insert(std::make_pair(startFrame, 0));
	
	std::multimap< int, FrameSE3* > priorityQueue;
	priorityQueue.insert(std::make_pair(0, startFrame));
	while (! priorityQueue.empty())
	{
		auto it = priorityQueue.begin();
		int length = it->first;
		FrameSE3* frame = it->second;
		...
		for (FrameSE3* neighbor : frame->neighbors)
		{
