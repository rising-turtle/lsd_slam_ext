...
double g_max_diff_constant = 30.0*30.; // 30. 10.
double g_max_diff_grad_mult = 0.5*0.5; 
double g_min_goodperbad_pixel = 0.5; // 0.3

bool useFabMap = false;
bool doSlam = true;
bool doKFReActivation = false; // true
bool doMapping = true;
...
bool pauseForTracking = false;
bool saveGraphStructure = false;
...
//		autoRunWithinFrame = !autoRunWithinFrame; 	// disabled... only use for debugging & if you really, really know what you are doing
	...
	case 'l': case 'L':
		...
		break;
	case 'g': case 'G':
		printf("Save G2O structure\n");
		saveGraphStructure = true; 
		break;
        case 32:
                printf("Pause to track the sr_frames!\n");
                pauseForTracking = !pauseForTracking; 
                break; 
	}
