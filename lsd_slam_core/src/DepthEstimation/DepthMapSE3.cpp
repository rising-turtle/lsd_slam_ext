...
DepthMapSE3::DepthMapSE3(int w, int h, const Eigen::Matrix3f& K)
{
	width = w;
	height = h;
...
void DepthMapSE3::observeDepthRow(int yMin, int yMax, RunningStats* stats)
{
...
void DepthMapSE3::observeDepth()
{

	threadReducer.reduce(boost::bind(&DepthMapSE3::observeDepthRow, this, _1, _2, _3), 3, height-3, 10);
...
bool DepthMapSE3::makeAndCheckEPL(const int x, const int y, const FrameSE3* const ref, float* pepx, float* pepy, RunningStats* const stats)
{
	int idx = x+y*width;
...
bool DepthMapSE3::observeDepthCreate(const int &x, const int &y, const int &idx, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;

	FrameSE3* refFrame = activeKeyFrameIsReactivated ? newest_referenceFrame : oldest_referenceFrame;
...
bool DepthMapSE3::observeDepthUpdate(const int &x, const int &y, const int &idx, const float* keyFrameMaxGradBuf, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;
	FrameSE3* refFrame;
...
void DepthMapSE3::propagateDepth(FrameSE3* new_keyframe)
{
...
void DepthMapSE3::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats)
{
...
void DepthMapSE3::regularizeDepthMapFillHoles()
{
	...
	threadReducer.reduce(boost::bind(&DepthMapSE3::regularizeDepthMapFillHolesRow, this, _1, _2, _3), 3, height-2, 10);
...
void DepthMapSE3::buildRegIntegralBufferRow1(int yMin, int yMax, RunningStats* stats)
{
...
void DepthMapSE3::buildRegIntegralBuffer()
{
	threadReducer.reduce(boost::bind(&DepthMapSE3::buildRegIntegralBufferRow1, this, _1, _2,_3), 0, height);
...
template<bool removeOcclusions> void DepthMapSE3::regularizeDepthMapRow(int validityTH, int yMin, int yMax, RunningStats* stats)
{
...
template void DepthMapSE3::regularizeDepthMapRow<true>(int validityTH, int yMin, int yMax, RunningStats* stats);
template void DepthMapSE3::regularizeDepthMapRow<false>(int validityTH, int yMin, int yMax, RunningStats* stats);


void DepthMapSE3::regularizeDepthMap(bool removeOcclusions, int validityTH)
{
	...
	if(removeOcclusions)
		threadReducer.reduce(boost::bind(&DepthMapSE3::regularizeDepthMapRow<true>, this, validityTH, _1, _2, _3), 2, height-2, 10);
	else
		threadReducer.reduce(boost::bind(&DepthMapSE3::regularizeDepthMapRow<false>, this, validityTH, _1, _2, _3), 2, height-2, 10);
...
void DepthMapSE3::initializeRandomly(FrameSE3* new_frame)
{
...
void DepthMapSE3::setFromExistingKF(FrameSE3* kf)
{
...
void DepthMapSE3::initializeFromGTDepth(FrameSE3* new_frame)
{
...
void DepthMapSE3::resetCounters()
{
...
void DepthMapSE3::updateKeyframe(std::deque< std::shared_ptr<FrameSE3> > referenceFrames)
{
	assert(isValid());
	...
	for(std::shared_ptr<FrameSE3> frame : referenceFrames)
	{
...
void DepthMapSE3::createKeyFrame(FrameSE3* new_keyframe)
{
	assert(isValid());
	...
	activeKeyFrame->pose->thisToParent_raw = oldToNew_SE3.inverse(); // sim3FromSE3(oldToNew_SE3.inverse(), rescaleFactor);
...
void DepthMapSE3::addTimingSample()
{
	struct timeval now;
...
void DepthMapSE3::finalizeKeyFrame()
{
	assert(isValid());
...
int DepthMapSE3::debugPlotDepthMap()
{
...
inline float DepthMapSE3::doLineStereo(
