 ...
 * July. 4, 2016 David Z 
 * 
 * extend Frame structure to include se3 structure 
 *
 * */
...
public:
	...
	friend class FrameMemorySE3;


	FrameSE3(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image, float* gt_p = 0);

	FrameSE3(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const float* image, float* gt_p = 0);
	...
	void prepareForStereoWith(FrameSE3* other, SE3 thisToOther, const Eigen::Matrix3f& K, const int level);
	
        // compute x, y, z given a index  
        bool getPointXYZ(int idx, Eigen::Vector3f&);
	...
	void setPermaRef(TrackingReferenceSE3* reference);
	...
	{
		return FrameMemorySE3::getInstance().activateFrame(this);
	}
	...
	// FramePoseStruct* pose;
        FramePoseStructSE3* pose;
        Sophus::SE3f gt_pose;  // ground truth pose 
        bool b_has_gt_pose; 
	...
	// Frame* getTrackingParent() { return pose->trackingParent->frame;}
        FrameSE3* getTrackingParent() { return pose->trackingParent->frame;}
	...
	std::unordered_set< FrameSE3*, std::hash<FrameSE3*>, std::equal_to<FrameSE3*>,
		Eigen::aligned_allocator< FrameSE3* > > neighbors;
	...
	// 	Eigen::aligned_allocator< std::pair<const Frame*,Sim3> > > trackingFailed;
        ...
        std::unordered_multimap< FrameSE3*, SE3, std::hash<FrameSE3*>, std::equal_to<FrameSE3*>,
	 	Eigen::aligned_allocator< std::pair<const FrameSE3*,SE3> > > trackingFailed;
...
inline int FrameSE3::id() const
{
	return data.id;
}

inline int FrameSE3::width(int level) const
{
	return data.width[level];
}

inline int FrameSE3::height(int level) const
{
	return data.height[level];
}

inline const Eigen::Matrix3f& FrameSE3::K(int level) const
{
	return data.K[level];
}
inline const Eigen::Matrix3f& FrameSE3::KInv(int level) const
{
	return data.KInv[level];
}
inline float FrameSE3::fx(int level) const
{
	return data.fx[level];
}
inline float FrameSE3::fy(int level) const
{
	return data.fy[level];
}
inline float FrameSE3::cx(int level) const
{
	return data.cx[level];
}
inline float FrameSE3::cy(int level) const
{
	return data.cy[level];
}
inline float FrameSE3::fxInv(int level) const
{
	return data.fxInv[level];
}
inline float FrameSE3::fyInv(int level) const
{
	return data.fyInv[level];
}
inline float FrameSE3::cxInv(int level) const
{
	return data.cxInv[level];
}
inline float FrameSE3::cyInv(int level) const
{
	return data.cyInv[level];
}

inline double FrameSE3::timestamp() const
{
	return data.timestamp;
}


inline float* FrameSE3::image(int level)
{
	if (! data.imageValid[level])
		require(IMAGE, level);
	return data.image[level];
}
inline const Eigen::Vector4f* FrameSE3::gradients(int level)
{
	if (! data.gradientsValid[level])
		require(GRADIENTS, level);
	return data.gradients[level];
}
inline const float* FrameSE3::maxGradients(int level)
{
	if (! data.maxGradientsValid[level])
		require(MAX_GRADIENTS, level);
	return data.maxGradients[level];
}
inline bool FrameSE3::hasIDepthBeenSet() const
{
	return data.hasIDepthBeenSet;
}
inline const float* FrameSE3::idepth(int level)
{
	if (! data.hasIDepthBeenSet)
	{
		printfAssert("FrameSE3::idepth(): idepth has not been set yet!");
		return nullptr;
	}
...
inline const unsigned char* FrameSE3::validity_reAct()
{
	if( !data.reActivationDataValid)
		return 0;
	return data.validity_reAct;
}
inline const float* FrameSE3::idepth_reAct()
{
	if( !data.reActivationDataValid)
		return 0;
	return data.idepth_reAct;
}
inline const float* FrameSE3::idepthVar_reAct()
{
	if( !data.reActivationDataValid)
		return 0;
	return data.idepthVar_reAct;
}
inline const float* FrameSE3::idepthVar(int level)
{
	if (! data.hasIDepthBeenSet)
	{
		printfAssert("FrameSE3::idepthVar(): idepth has not been set yet!");
		return nullptr;
	}
...
inline bool* FrameSE3::refPixelWasGood()
{
	if( data.refPixelWasGood == 0)
	{
		...
		if(data.refPixelWasGood == 0)
		{
			...
			data.refPixelWasGood = (bool*)FrameMemorySE3::getInstance().getBuffer(sizeof(bool) * width * height);
...
inline bool* FrameSE3::refPixelWasGoodNoCreate()
{
	return data.refPixelWasGood;
}

inline void FrameSE3::clear_refPixelWasGood()
{
	FrameMemorySE3::getInstance().returnBuffer(reinterpret_cast<float*>(data.refPixelWasGood));
