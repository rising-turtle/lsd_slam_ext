...
int privateFrameSE3AllocCount = 0;





FrameSE3::FrameSE3(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image, float* gt_p)
{
	initialize(id, width, height, K, timestamp);
	
	data.image[0] = FrameMemorySE3::getInstance().getFloatBuffer(data.width[0]*data.height[0]);
	...
	privateFrameSE3AllocCount++;

	if(enablePrintDebugInfo && printMemoryDebugInfo)
		printf("ALLOCATED frame %d, now there are %d\n", this->id(), privateFrameSE3AllocCount);

        setGT(gt_p);
}

FrameSE3::FrameSE3(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const float* image, float * gt_p)
{
	initialize(id, width, height, K, timestamp);
...
void FrameSE3::setGT(float *gt_p)
{
     Eigen::Matrix4f T = Eigen::Matrix4f::Identity();
        if(gt_p != 0)
        {
            Eigen::Quaternionf q = Eigen::Quaternionf(gt_p[6], gt_p[3], gt_p[4], gt_p[5]);  // qw, qx, qy, qz
            T.block(0,0,3,3) = q.matrix(); 
            T.block(0,3,3,1) = Eigen::Vector3f(gt_p[0], gt_p[1], gt_p[2]);
            b_has_gt_pose = true;

            // printf("ROSOutput3DWrapper.cpp: frame %d at %lf has gt %f %f %f %f %f %f %f\n", id, f->timestamp(), gp.translation()[0], gp.translation()[1], 
            //    gp.translation()[2], gp.unit_quaternion().x(), gp.unit_quaternion().y(), gp.unit_quaternion().z(), gp.unit_quaternion().w());

        }else{
            b_has_gt_pose = false;
        }
        gt_pose = Sophus::SE3f(T); 
}
...
FrameSE3::~FrameSE3()
{
	...
	FrameMemorySE3::getInstance().deactivateFrame(this);
	...
	{
		FrameMemorySE3::getInstance().returnBuffer(data.image[level]);
		FrameMemorySE3::getInstance().returnBuffer(reinterpret_cast<float*>(data.gradients[level]));
		FrameMemorySE3::getInstance().returnBuffer(data.maxGradients[level]);
		FrameMemorySE3::getInstance().returnBuffer(data.idepth[level]);
		FrameMemorySE3::getInstance().returnBuffer(data.idepthVar[level]);
	}

	FrameMemorySE3::getInstance().returnBuffer((float*)data.validity_reAct);
	FrameMemorySE3::getInstance().returnBuffer(data.idepth_reAct);
	FrameMemorySE3::getInstance().returnBuffer(data.idepthVar_reAct);
...
void FrameSE3::takeReActivationData(DepthMapPixelHypothesis* depthMap)
{
	boost::shared_lock<boost::shared_mutex> lock = getActiveLock();

	if(data.validity_reAct == 0)
		data.validity_reAct = (unsigned char*) FrameMemorySE3::getInstance().getBuffer(data.width[0]*data.height[0]);

	if(data.idepth_reAct == 0)
		data.idepth_reAct = FrameMemorySE3::getInstance().getFloatBuffer((data.width[0]*data.height[0]));

	if(data.idepthVar_reAct == 0)
		data.idepthVar_reAct = FrameMemorySE3::getInstance().getFloatBuffer((data.width[0]*data.height[0]));
...
/*
void FrameSE3::setPermaRef(TrackingReference* reference)
{
...
void FrameSE3::setPermaRef(TrackingReferenceSE3* reference)
{
	assert(reference->frameID == id());
	reference->makePointCloud(QUICK_KF_CHECK_LVL);
...
void FrameSE3::calculateMeanInformation()
{
	return;
...
void FrameSE3::setDepth(const DepthMapPixelHypothesis* newDepth)
{
	...
	if(data.idepth[0] == 0)
		data.idepth[0] = FrameMemorySE3::getInstance().getFloatBuffer(data.width[0]*data.height[0]);
	if(data.idepthVar[0] == 0)
		data.idepthVar[0] = FrameMemorySE3::getInstance().getFloatBuffer(data.width[0]*data.height[0]);
...
bool FrameSE3::getPointXYZ(int idx, Eigen::Vector3f& pt)
{
  if(data.idepth[0] == 0)
    return false;
    int v = idx/data.width[0]; 
    int u = idx - v*data.width[0]; 
    float d = data.idepth[0][idx]; 

    float fxV = 1./data.fx[0];
    float fyV = 1./data.fy[0];
    pt(2) = 1./d; 
    pt(0) = (u-data.cx[0])*fxV*pt(2);  // x
    pt(1) = (v-data.cy[0])*fyV*pt(2);  // y
    return true;
}

void FrameSE3::setDepthFromGroundTruth(const float* depth, float cov_scale)
{
	boost::shared_lock<boost::shared_mutex> lock = getActiveLock();
...
void FrameSE3::prepareForStereoWith(FrameSE3* other, SE3 thisToOther, const Eigen::Matrix3f& K, const int level)
{
...
void FrameSE3::require(int dataFlags, int level)
{
...
void FrameSE3::release(int dataFlags, bool pyramidsOnly, bool invalidateOnly)
{
...
bool FrameSE3::minimizeInMemory()
{
...
void FrameSE3::initialize(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp)
{
	data.id = id;
	
	// pose = new FramePoseStruct(this);
        pose = new FramePoseStructSE3(this);
...
// 		data.refIDValid[level] = false;
	...
	lastConstraintTrackedCamToWorld =  SE3(); //Sim3();

	isActive = false;
}

void FrameSE3::setDepth_Allocate()
{
	return;
}

void FrameSE3::buildImage(int level)
{
	if (level == 0)
	{
	...
	if (data.image[level] == 0)
		data.image[level] = FrameMemorySE3::getInstance().getFloatBuffer(data.width[level] * data.height[level]);
...
void FrameSE3::releaseImage(int level)
{
	if (level == 0)
	{
...
void FrameSE3::buildGradients(int level)
{
	require(IMAGE, level);
	boost::unique_lock<boost::mutex> lock2(buildMutex);
	...
	if(data.gradients[level] == 0)
		data.gradients[level] = (Eigen::Vector4f*)FrameMemorySE3::getInstance().getBuffer(sizeof(Eigen::Vector4f) * width * height);
	...
	for(; img_pt < img_pt_max; img_pt++, gradxyii_pt++)
	{
		val_p1 = *(img_pt+1);
                ...
                if(val_p1 == 0 || val_m1 == 0 || val_00 == 0 || *(img_pt+width) == 0 || *(img_pt-width) == 0)
                {
		  *((float*)gradxyii_pt) = 0.;
...
void FrameSE3::releaseGradients(int level)
{
	FrameMemorySE3::getInstance().returnBuffer(reinterpret_cast<float*>(data.gradients[level]));
	data.gradients[level] = 0;
}
...
void FrameSE3::buildMaxGradients(int level)
{
	...
	if (data.maxGradients[level] == 0)
		data.maxGradients[level] = FrameMemorySE3::getInstance().getFloatBuffer(width * height);
	
	float* maxGradTemp = FrameMemorySE3::getInstance().getFloatBuffer(width * height);
	...
	FrameMemorySE3::getInstance().returnBuffer(maxGradTemp);

	data.maxGradientsValid[level] = true;
}

void FrameSE3::releaseMaxGradients(int level)
{
	FrameMemorySE3::getInstance().returnBuffer(data.maxGradients[level]);
	data.maxGradients[level] = 0;
}

void FrameSE3::buildIDepthAndIDepthVar(int level)
{
	...
	if (data.idepth[level] == 0)
		data.idepth[level] = FrameMemorySE3::getInstance().getFloatBuffer(width * height);
	if (data.idepthVar[level] == 0)
		data.idepthVar[level] = FrameMemorySE3::getInstance().getFloatBuffer(width * height);
...
void FrameSE3::releaseIDepth(int level)
{
	if (level == 0)
	{
...
void FrameSE3::releaseIDepthVar(int level)
{
	if (level == 0)
	{
...
void FrameSE3::printfAssert(const char* message) const
{
	assert(!message);
	printf("%s\n", message);
}
